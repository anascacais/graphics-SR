...
""" This script creates a network plot with the main players in the given research field
    (adapted from https://github.com/anascacais/research_network & adapted from Olivia Tanuwidjaja's post on
...
def main(filepath, title, use_affiliations=True, remove_single=True, colormap='Blues'):

    G = get_network_body(filepath, remove_single)

    node_size = get_node_size_by_papers(G, filepath)
    legend, node_color = get_affiliations(
        G, filepath, use_affiliations, colormap)

    plot_network(G, node_size, node_color, legend,
                 remove_single, colormap, title)


if __name__ == '__main__':
    main(filepath='aux_files/input_data.xlsx',
         title='Algorithmic approaches: How different data inputs are combined',
         remove_single=False
         )
